...
음성이 사용자의 음성인지, 타인의 음성인지 구별하는 머신러닝 분류 모델을 통해 보안을 강화할 필요가 있다. 본 연구는 1. 음성에서 성별과 연령대
를 분류하는 성능이 가장 높은 머신러닝 모델을 찾는 연구와 2. 머신러닝 모델과 통계적 방법을 앙상블하여 유의미한 성능 개선이 있는지 확인하는
연구로 진행된다.
...
음원 읽기와 푸리에 변환에는 파이썬의 librosa 패키지를 사용했으며 음원에서 비어있는 부분을 제거한 후 시간에 따라 20개 주파수 대역별로 세기를
나타낸 MFCC 벡터를 생성했다.

<img src = "./img/1.MFCC_sample.jpg" width="50%">

가로축은 시간, 세로축은 주파수 대역을 나타낸 것이다. 낮은 주파수 대역의 음압이 낮고(파란색) 높은 주파수 대역의 음압이 높은 것을(빨간색)
확인할 수 있다. 읽기 오류가 발생하는 4개의 파일을 제외한 87,045개 음원에서 MFCC 벡터를 생성하고 음압이 미약한 0번 인덱스를 제거한 뒤 주파수
대역별로 평균값을 구했다. 음원별로 (1x19) 크기의 벡터가 생성되었으며 총 데이터의 크기는 (87045, 19)이다. 생성한 데이터는 pickle 모듈을
사용해 .pkl 파일로 저장했다.
...
<img src = "./img/5.NHR_table.PNG" width="80%">

###### (서윤정, 신지영. (2018). 모음 연장 발성이 보이는 연령대별 음향음성학적 특성 연구. 말소리와 음성과학, 10(4), pp. 67-76.)
  
  
2. 음성의 특성 값을 추출해 특정 카테고리에 속할 확률을 계산한다.
- 특성 값 추출에는 parselmouth 패키지를 사용한다. NHR은 추출한 HNR을 변환식에 대입하여 구한다.
...
- 각 열을 각 열의 합으로 나눈다. jitter만 봤을 때 이 샘플 데이터는 30대 남성일 확률이 21.7%이다.
...
- 각 열을 모두 더하고 특성의 개수(=4)로 나눠 각 카테고리에 속할 확률을 구한다. 이 샘플 데이터는 20대 여성일 확률이 23.7%로 가장 높다.
...
결론
--------------
1. 음성의 성별과 연령대를 분류하는 성능이 가장 우수한 머신러닝 모델은 랜덤 포레스트로 트리의 수는 많으면 많을수록, 트리의 깊이는 25 부근에서
높은 성능을 보인다.
2. 통계적 분류 모델과 앙상블한 결과 SVM을 제외한 머신러닝 모델의 분류 성능이 소폭 상승했다.
3. SVM의 분류 성능은 통계적 분류 모델과 앙상블한 결과 90%대에서 40%대로 크게 하락했다.
4. SVM을 제외하면 통계적 분류 모델과 머신러닝 모델을 앙상블한 결과 성능이 향상된다는 가설을 채택할 수 있다.
5. SVM을 포함하면 통계적 분류 모델과 머신러닝 모델을 앙상블한 결과 성능이 향상된다는 가설은 기각된다.
